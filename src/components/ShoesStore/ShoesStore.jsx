...
export default class ShoesStore extends Component {
  ...
  addToCart = (prodClick) => {
    ...
    let prodCartClick = { ...prodClick, count: 1 };
    let cartUpdate = this.state.arrCart;

    //Xử lý khi thêm 1 sp
    //Check sp đã có trong giỏ hàng chưa
    let prodCheck = cartUpdate.find((prod) => prod.id === prodClick.id);
    if (prodCheck) {
      //dã có thì tăng số lượng lên 1
      prodCheck.count += 1;
    } else {
      cartUpdate.push(prodCartClick);
    }
    //setState
    this.setState({
      arrCart: cartUpdate,
  ...
  deleteCart = (idProdClick) => {
    // dựa vào mã để cập nhật lại this.state
    console.log(idProdClick);
    let cartUpdate = this.state.arrCart;

    let index = cartUpdate.findIndex((prod) => prod.id === idProdClick);
    if (index != -1) {
  ...
  updateCart = (idProdClick, count) => {
    console.log(idProdClick, count);
    let cartUpdate = this.state.arrCart;
    //tìm ra sp dựa vào mã click trong giỏ hàng
    let prodClick = cartUpdate.find((prod) => prod.id === idProdClick);
    if (prodClick) {
      prodClick.count += count;
      // xử lý khi số lượng giảm đến 0
      if (prodClick.count < 1) {
        if (window.confirm("Bạn có muốn xoá sản phẩm này không?")) {
          this.deleteCart(idProdClick);
          //sau khi xoá xong thì dừng hàm
          return;
        } else {
          prodClick.count += 1;
        }
  ...
  render() {
    return (
      <div className="container">
        ...
        />
        <Model prodDetail={this.state.prodDetail} />
